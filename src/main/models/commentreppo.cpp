...
Comment CommentRepo::addComment(Comment comment) {
    ...
    if (r != 0) {
        ...
        Comment newComment = searchCommentById(static_cast<int>(lastRowId));
...
std::vector<Comment> CommentRepo::getCommentsByTweetId(int tweetId) {
  ...
  for (auto commentData : result) {
      Comment c(stoi(commentData["id"]), stoi(commentData["author_id"]), 
...
Comment CommentRepo::searchCommentById(int id) {
  std::string sql;
  sql += "SELECT * FROM comment WHERE id = " + std::to_string(id) + ";";
  std::vector<std::map<std::string, std::string>> result = executeSelect(_db, sql);

  Comment c(stoi(result[0]["id"]), stoi(result[0]["author_id"]), 
